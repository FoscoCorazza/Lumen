...
public abstract class Obstacle extends Segment {
    ...
    protected boolean gammaInclusive = true;
    protected boolean thetaInclusive = true;
    ...
    protected abstract void drawSingleTerminator(Canvas canvas, Path polygon, GridDot dot, float r);
    ...
    protected void drawLineTerminators(Canvas canvas, int gx, int gy, int tx, int ty, float r) {
        drawSingleTerminator(canvas, polygonGamma, gamma.gridDot(), Consts.lineTerminatorRadius);
        if(!length().isZero())
            drawSingleTerminator(canvas, polygonTheta, theta.gridDot(), Consts.lineTerminatorRadius);
    }
    ...
    public boolean isGammaInclusive() {
        return gammaInclusive;
    }

    public void setGammaInclusive(boolean gammaInclusive) {
        this.gammaInclusive = gammaInclusive;
    }

    public boolean isThetaInclusive() {
        return thetaInclusive;
    }

    public void setThetaInclusive(boolean thetaInclusive) {
        this.thetaInclusive = thetaInclusive;
    }
}
